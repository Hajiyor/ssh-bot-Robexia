...
def init_db_sync():
    ...
    try:
        ...
        # تنظیمات داخلی (AES key، welcome text، و...)
        c.execute("""
...
async def log_ssh_connect(user_id: int, host: str) -> int:
    """ثبت شروع اتصال SSH، برمی‌گرداند id لاگ"""
    async with aiosqlite.connect(config.DB_PATH) as db:
        cur = await db.execute(
            "INSERT INTO ssh_logs (user_id, host) VALUES (?, ?)",
            (user_id, host)
        )
        await db.commit()
...
async def log_ssh_disconnect(log_id: int) -> None:
    ...
    async with aiosqlite.connect(config.DB_PATH) as db:
        await db.execute(
            "UPDATE ssh_logs SET disconnected_at=CURRENT_TIMESTAMP WHERE id=?",
            (log_id,)
        )
        await db.commit()
...
async def get_user_hosts(user_id: int) -> List[Dict]:
    async with aiosqlite.connect(config.DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        cur = await db.execute(
            "SELECT id,name,host,port,username,auth_type,created_at FROM hosts "
...
async def add_host(user_id: int, name: str, host: str, port: int,
                   ...
                   password_enc=None, key_enc=None, key_passphrase_enc=None) -> Optional[int]:
    if await count_user_hosts(user_id) >= MAX_HOSTS_PER_USER:
        return None
    async with aiosqlite.connect(config.DB_PATH) as db:
        cur = await db.execute("""
            INSERT INTO hosts (user_id,name,host,port,username,auth_type,
